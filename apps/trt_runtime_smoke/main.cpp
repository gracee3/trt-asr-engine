...
struct TrtDeleter {
  ...
  void operator()(T* p) const noexcept {
    // TensorRT 10 uses standard virtual destructors (destroy() is removed).
    delete p;
  }
};
...
static void print_engine_bindings(const Engine& e) {
  ...
  std::cout << "=== " << e.name << " io_tensors ===\n";
  for (int i = 0; i < nb; ++i) {
    const char* tn = e.engine->getIOTensorName(i);
    const auto mode = e.engine->getTensorIOMode(tn);
    ...
    std::cout << "  [" << i << "] " << (is_input ? "IN " : "OUT") << " " << tn << " dtype=" << dtype_str(dt)
...
static void set_shape_or_throw(Engine& e, const char* binding_name, const std::vector<int32_t>& dims) {
...
#if NV_TENSORRT_MAJOR >= 10
  const auto mode = e.engine->getTensorIOMode(binding_name);
  if (mode != nvinfer1::TensorIOMode::kINPUT) throw std::runtime_error(std::string("Tensor not input: ") + binding_name);
  if (!e.ctx->setInputShape(binding_name, d)) throw std::runtime_error(std::string("setInputShape failed for ") + binding_name);
#else
  ...
  if (!e.ctx->setBindingDimensions(idx, d)) throw std::runtime_error(std::string("setBindingDimensions failed for ") + binding_name);
#endif
}
...
static void allocate_buffers_for_current_shapes(
    ...
    std::vector<void*>& legacy_bindings,
    std::map<std::string, void*>& tensor_ptrs,
    cudaStream_t stream) {
#if NV_TENSORRT_MAJOR >= 10
  (void)legacy_bindings;
  const int nb = e.engine->getNbIOTensors();
  ...
  const int nb = e.engine->getNbBindings();
  bufs.resize(static_cast<size_t>(nb));
  legacy_bindings.resize(static_cast<size_t>(nb), nullptr);
  ...
  for (int i = 0; i < nb; ++i) {
    ...
    if (b.ptr) {
      if (b.bytes >= bytes) {
        legacy_bindings[static_cast<size_t>(i)] = b.ptr;
        continue;
      }
      cuda_check(cudaFree(b.ptr), "cudaFree");
      b.ptr = nullptr;
...
static void run_once(Engine& e, cudaStream_t stream, std::vector<void*>& legacy_bindings) {
  auto t0 = std::chrono::high_resolution_clock::now();
#if NV_TENSORRT_MAJOR >= 10
  if (!e.ctx->enqueueV3(stream)) throw std::runtime_error("enqueueV3 failed for engine: " + e.name);
#else
  if (!e.ctx->enqueueV2(legacy_bindings.data(), stream, nullptr)) throw std::runtime_error("enqueueV2 failed for engine: " + e.name);
#endif
...
static void check_output_non_nan(Engine& e, const char* out_name, int max_values_to_check, cudaStream_t stream,
                                 const std::vector<void*>& legacy_bindings,
                                 const std::map<std::string, void*>& tensor_ptrs) {
#if NV_TENSORRT_MAJOR >= 10
  const auto mode = e.engine->getTensorIOMode(out_name);
  if (mode != nvinfer1::TensorIOMode::kOUTPUT) throw std::runtime_error(std::string("Expected output tensor: ") + out_name);
  const auto dims = e.ctx->getTensorShape(out_name);
  const auto dt = e.engine->getTensorDataType(out_name);
  auto it = tensor_ptrs.find(out_name);
  if (it == tensor_ptrs.end()) throw std::runtime_error(std::string("No device pointer for tensor: ") + out_name);
  ...
  void* dev_ptr = legacy_bindings[static_cast<size_t>(idx)];
#endif
  ...
  if (dt == nvinfer1::DataType::kFLOAT) {
    cuda_check(cudaMemcpyAsync(host.data(), dev_ptr, k * sizeof(float), cudaMemcpyDeviceToHost, stream),
               "cudaMemcpyAsync");
    cuda_check(cudaStreamSynchronize(stream), "cudaStreamSynchronize");
  } else {
    // FP16 -> convert via CPU half decode (very small check). This is a smoke check, not perf-critical.
    std::vector<uint16_t> h16(k, 0);
    cuda_check(cudaMemcpyAsync(h16.data(), dev_ptr, k * sizeof(uint16_t), cudaMemcpyDeviceToHost, stream),
               "cudaMemcpyAsync");
    cuda_check(cudaStreamSynchronize(stream), "cudaStreamSynchronize");
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    std::map<std::string, void*> enc_tensors, pred_tensors, joint_tensors;

    allocate_buffers_for_current_shapes(enc, enc_bufs, enc_bindings, enc_tensors, stream);
    allocate_buffers_for_current_shapes(pred, pred_bufs, pred_bindings, pred_tensors, stream);
    allocate_buffers_for_current_shapes(joint, joint_bufs, joint_bindings, joint_tensors, stream);
    cuda_check(cudaStreamSynchronize(stream), "cudaStreamSynchronize");
    ...
    // Fill required INT64 inputs deterministically.
    {
      int64_t host_len = 64;
#if NV_TENSORRT_MAJOR >= 10
      auto it = enc_tensors.find("length");
      if (it == enc_tensors.end()) throw std::runtime_error("Missing tensor pointer for encoder:length");
      cuda_check(cudaMemcpyAsync(it->second, &host_len, sizeof(host_len), cudaMemcpyHostToDevice, stream), "cudaMemcpyAsync(length)");
#else
...
#if NV_TENSORRT_MAJOR >= 10
      auto it2 = pred_tensors.find("y");
      if (it2 == pred_tensors.end()) throw std::runtime_error("Missing tensor pointer for predictor:y");
      cuda_check(cudaMemcpyAsync(it2->second, &host_y, sizeof(host_y), cudaMemcpyHostToDevice, stream), "cudaMemcpyAsync(y)");
#else
    ...
    check_output_non_nan(enc, "encoder_output", 32, stream, enc_bindings, enc_tensors);
    check_output_non_nan(pred, "g", 32, stream, pred_bindings, pred_tensors);
    check_output_non_nan(joint, "joint_output", 32, stream, joint_bindings, joint_tensors);
