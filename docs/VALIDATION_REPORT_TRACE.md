...
### Tail padding policy (runtime)
- Tail chunks with `T_actual < 41` are zero‑padded to the streaming profile target (`41` for chunk0, `57` for chunk>=1).
- The `length` input remains `T_actual`, and decoder consumes only `T_enc` steps.

### TRT trace run (debug steps capped, untrimmed WAV)
```bash
...
rust/target/debug/cli \
  ...
  --stream-sim 0.5 \
  eval/wav/librispeech_dev_gate/dev-clean/1272-128104-0000.wav \
  > artifacts/logs/tdt_1utt_trt_fp32_noTF32_untrimmed.log \
  2> artifacts/logs/tdt_1utt_trt_fp32_noTF32_untrimmed.err

cp .cursor/debug.log artifacts/trace/tdt_1utt_trt_fp32_noTF32_untrimmed.jsonl
```
...
python tools/stt_suite/run_suite.py \
  ...
  --variants base \
  --rounds 1 \
  --output-dir artifacts/logs/tdt_10utt_fp32_noTF32_untrimmed
```
